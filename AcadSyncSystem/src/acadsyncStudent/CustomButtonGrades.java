...
class CustomButtonGrades extends JButton {
	private static final long serialVersionUID = 1L;
	private int cornerRadius; // Corner radius field
	private Color hoverBackground;   // Hover background color

    public CustomButtonGrades(String text, int cornerRadius) {
        super(text);
        this.cornerRadius = cornerRadius;
        this.hoverBackground = new Color(255, 116, 73);   // Hover background
        setUI(new javax.swing.plaf.basic.BasicButtonUI()); // Basic UI allows customization
        setBackground(new Color(255, 217, 205)); // Default background
        ...
        addMouseListener(new MouseAdapter() {
            ...
            public void mouseExited(MouseEvent e) { 
                setBackground(new Color(255, 217, 205)); // Change to hover background
            }
        });
    }
    ...
    protected void paintComponent(Graphics g) {
        ...
        if (getModel().isPressed()) {
            g2.setColor(new Color(255, 217, 205)); // Pressed state color
        } else {
