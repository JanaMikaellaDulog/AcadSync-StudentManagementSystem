...
public class CustomButtonCalendar extends JButton {
    /**
	 * 
	 */
    ...
    public CustomButtonCalendar(String text, int cornerRadius) {
        super(text);
        this.cornerRadius = cornerRadius;
        this.hoverBackground = new Color(185, 142, 47);   // Hover background
        setUI(new javax.swing.plaf.basic.BasicButtonUI()); // Basic UI allows customization
        setBackground(new Color(253, 181, 21)); // Default background
        setForeground(new Color(0, 0, 0));// Default foreground
        ...
        addMouseListener(new MouseAdapter() {
            ...
            public void mouseExited(MouseEvent e) { 
                setBackground(new Color(253, 181, 21)); // Change to hover background
            }
        });
    }
    ...
    protected void paintComponent(Graphics g) {
        ...
        if (getModel().isPressed()) {
            g2.setColor(new Color(253, 181, 21)); // Pressed state color
        } else {
    ...
    protected void paintBorder(Graphics g) {
        Graphics2D g2 = (Graphics2D) g;
        g2.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_ON); // Smooth edges
        g2.setStroke(new java.awt.BasicStroke(0));  // Set the stroke thickness (for LineBorder)
